...
from config import node_types_to_train, metadata, parse_args, create_saving_filename
...
def setup_experiment(metadata) -> tuple:
    ...
    # wandb.init(project=f'reproduce_meshgraphnet_{metadata["dataset_name"]}_pytorch', config=metadata)

    # Create savings directory
    # general_savings_dir = create_saving_filename(metadata)
    # os.makedirs(general_savings_dir, exist_ok=False)  # CHANGE TO FALSE!!!!!!!!!!
    general_saving_dir = './delete/'
    os.makedirs(general_saving_dir, exist_ok=True)
    ...
    return model, dataloader, val_dataloader, optimizer, scheduler, general_saving_dir
...
def main():
    ...
    # parsed_metadata = parse_args()
    # metadata.update(parsed_metadata)  # Update metadata in the whole directory

    with open('savings/cylinder_flow/run0/metadata.yaml', 'r') as f:
        metadata = yaml.safe_load(f)
        
    metadata['device'] = metadata['device'] if torch.cuda.is_available() else 'cpu'
    metadata['device'] = 'cuda:2'
    metadata['warm_up_batches'] = 10
